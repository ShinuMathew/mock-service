...
class CourseManager {
    ...
    async publishCourses(req) {
        try {
            ...
            logger.error(`Error occurred in publishCourses Manager :\n ${err}`)
            if (err['detail'].includes("exists")) {
                throw await this._constructFailureResponse(Error.CourseAlreadyRegistered, req)
            }
        }
    }
    ...
    async updateCourse(req) {
        try {
            ...
            if (result.length == 0)
                return await this._constructFailureResponse(Error.CourseNotFound, req)
            let param = [
                req.params.id,
                req.payload.course_name,
                req.payload.course_type
    ...
    async getSpecificCourse(req) {
        try {
            let result = await this.courseDBAccessor.getSpecificCourse(req.params.id)
            if (result.length === 0)
                throw await this._constructFailureResponse(Error.CourseNotFound, req)
            result = _.map(result, (row) => {
                return {
                    course_id: row.courseid,
                    course_name: row.coursename,
            ...
            logger.error(`Error occurred in getSpecificCourse Manager :\n ${err}`)
            throw err;
        }
    }
    ...
    async deleteSpecificCourse(req) {
        try {
            ...
            logger.error(`Error occurred in deleteSpecificCourse Manager :\n ${err}`)
            throw err;
        }
    }
